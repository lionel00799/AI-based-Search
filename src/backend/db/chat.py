...
from db.models import ChatMessage as DBChatMessage
from db.models import ChatThread as DBChatThread
from db.models import SearchResult as DBSearchResult
from schemas import (
...
from utils import DB_ENABLED


def create_chat_thread(*, session: Session, modelName: str):
    chat_thread = DBChatThread(modelName=modelName)
    ...
    *,
    ...
    if DB_ENABLED:
        if thread_id is None:
            thread = create_chat_thread(session=session, modelName=model)
            thread_id = thread.id
        else:
            thread_id = thread_id
